...
public class ClubServiceImpl implements ClubService {
    ...
    public ClubDto createClub(CreateClubRequest request, UUID userExternalId) {
        ...
        // ownerem jest profil (nie konto)
        MemberProfile ownerProfile = user.getMemberProfile();
        Club club = ClubMapper.fromCreateRequest(request);
        club = clubRepository.save(club);
        ClubMember ownerMembership = ClubMember.createOwner(club, ownerProfile);
    ...
    public ClubDto getByExternalId(UUID clubExternalId, UUID userExternalId) {
        ...
        log.info("User externalId={} is {} of club externalId={}", userExternalId, isMember ? "member" : "not member", clubExternalId);
        return isMember
    ...
    public void deleteClub(UUID clubExternalId, UUID userExternalId) {
        ...
        club.setStatus(ClubStatus.DELETED);
        Club saved = clubRepository.save(club);
        log.info("Deleted club externalId={}, name={}", saved.getExternalId(), saved.getName());
    }
    @Override
    @Transactional(readOnly = true)
